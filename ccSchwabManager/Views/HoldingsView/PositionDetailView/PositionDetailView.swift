...
struct PositionDetailView: View
{
    ...
    private func applySnapshot(_ snapshot: SecurityDataSnapshot)
    {
        ...
        self.quoteData = snapshot.quoteData
        self.computedATRValue = snapshot.atrValue
        self.taxLotData = snapshot.taxLotData
        self.computedSharesAvailableForTrading = snapshot.sharesAvailableForTrading
    ...
    private func fetchDataForSymbol(forceRefresh: Bool = false)
    {
        ...
        } else if let cachedSnapshot = SecurityDataCacheManager.shared.snapshot(for: symbol) {
        ...
        DispatchQueue.global(qos: .userInitiated).async
        {
            ...
            let snapshot = SecurityDataSnapshot(
                symbol: symbol,
                fetchedAt: Date(),
                priceHistory: fetchedPriceHistory,
                transactions: fetchedTransactions,
                quoteData: fetchedQuote,
                atrValue: fetchedATRValue,
                taxLotData: fetchedTaxLots,
                sharesAvailableForTrading: fetchedSharesAvailable
            )
